...
async.series([
  function(next) {
    ...
    for (var i = 0; i < 100; i++) {
      ...
      // uint64le
      var br = new vertcore.encoding.BufferWriter();
      var num = Math.round(Math.random() * 10000000000000);
      br.writeUInt64LEBN(new vertcore.crypto.BN(num));
      buffers.push(br.toBuffer());

      // hashes
      var data = vertcore.crypto.Hash.sha256sha256(new Buffer(32));
    ...
    function readUInt64LEBN() {
      ...
      var buf = buffers[c];
      var br = new vertcore.encoding.BufferReader(buf);
  ...
  function(next) {
    ...
    function vertcoreTest() {
      block1 = vertcore.Block.fromString(blockData);
    }
